...
class Accountant extends CI_Controller
{
    ...
    function index()
    {
        ...
        $data['page_title'] = get_phrase('contabil_pagina_principala');
    ...
    function invoice_add($task = "")
    {
        ...
        if ($task == "create") {
            ...
            $this->session->set_flashdata('message', get_phrase('factura_fiscala_salvata_cu_succes'));
            redirect(site_url('accountant/invoice_manage'), 'refresh');
        }
        
        $data['page_name']  = 'add_invoice';
        $data['page_title'] = get_phrase('facutura_fiscala');
        $this->load->view('backend/index', $data);
    }
    ...
    function invoice_manage($task = "", $invoice_id = "")
    {
        ...
        if ($task == "update") {
            ...
            $this->session->set_flashdata('message', get_phrase('factura_fiscala_acutalizata_cu_succes'));
            redirect(site_url('accountant/invoice_manage'), 'refresh');
        }
    ...
    function manage_profile($task = "")
    {
        ...
        if ($task == "change_password") {
            ...
            if ($password == $old_password && $new_password == $confirm_new_password) {
                ...
                $this->session->set_flashdata('message', get_phrase('parola_actualizata_cu_succes'));
                redirect(site_url('accountant/manage_profile'), 'refresh');
            } else {
                $this->session->set_flashdata('message', get_phrase('actualizarea_parolei_a_esuat'));
                redirect(site_url('accountant/manage_profile'), 'refresh');
            }
        }
    ...
    function form($task = "")
    {
        ...
        $data['page_title'] = get_phrase('creati_forma');
        $this->load->view('backend/index', $data);
    }
