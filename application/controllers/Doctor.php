...
class Doctor extends CI_Controller
{
    ...
    function index()
    {
        ...
        $data['page_title'] = get_phrase('doctor_pagina_principala');
    ...
    function patient($task = "", $patient_id = "")
    {
        ...
        if ($task == "create") {
            ...
            $this->session->set_flashdata('message', get_phrase('informati_pacient_salvata_cu_succes'));
            redirect(site_url('doctor/patient'), 'refresh');
        }
        
        if ($task == "update") {
            $this->crud_model->update_patient_info($patient_id);
            $this->session->set_flashdata('message', get_phrase('informati_pacient_actualizata_cu_succes'));
            redirect(site_url('doctor/patient'), 'refresh');
        }
    ...
    function medication_history($param1 = "", $prescription_id = "")
    {
        ...
        $patient_name              = $this->db->get_where('pacient', array(
            'patient_id' => $param1
        ...
        $data['page_title']        = get_phrase('istoricul_medical_al_:_') . $patient_name;
        $this->load->view('backend/index', $data);
    }
    ...
    function bed_allotment($task = "", $bed_allotment_id = "")
    {
        ...
        if ($task == "create") {
            ...
            $this->session->set_flashdata('message', get_phrase('informatile_alocari_paturilor_salvata_cu_succes'));
            redirect(site_url('doctor/bed_allotment'), 'refresh');
        }
        
        if ($task == "update") {
            $this->crud_model->update_bed_allotment_info($bed_allotment_id);
            $this->session->set_flashdata('message', get_phrase('informatile_alocari_paturilor_actualizata_cu_succes'));
            redirect(site_url('doctor/bed_allotment'), 'refresh');
        }
    ...
    function report($task = "", $report_id = "", $param3 = '')
    {
        ...
        if ($task == "update") {
            ...
            $this->session->set_flashdata('message', get_phrase('informati_raport_actualzat_cu_succes'));
            redirect(site_url('doctor/report'), 'refresh');
        }
        ...
        if ($task == "delete_report_file") {
            ...
            $this->session->set_flashdata('message', get_phrase('fisiere_sterse_cu_succes'));
            redirect(site_url('doctor/report'), 'refresh');
        }
    ...
    function manage_profile($task = "")
    {
        ...
        if ($task == "change_password") {
            ...
            if ($password == $old_password && $new_password == $confirm_new_password) {
                ...
                $this->session->set_flashdata('message', get_phrase('informati_parola_actualizata_cu_succes'));
                redirect(site_url('doctor/manage_profile'), 'refresh');
            } else {
    ...
    function appointment($task = "", $appointment_id = "")
    {
        ...
        if ($task == "create") {
            ...
            $this->session->set_flashdata('message', get_phrase('informati_programare_salvata_cu_succes'));
            redirect(site_url('doctor/appointment'), 'refresh');
        }
        
        if ($task == "update") {
            $this->crud_model->update_appointment_info($appointment_id);
            $this->session->set_flashdata('message', get_phrase('informati_programare_actualizata_cu_succes'));
            redirect(site_url('doctor/appointment'), 'refresh');
        }
        ...
        $data['page_title']       = get_phrase('programare');
        $this->load->view('backend/index', $data);
    }
    ...
    function appointment_requested($task = "", $appointment_id = "")
    {
        ...
        if ($task == "approve") {
            ...
            $this->session->set_flashdata('message', get_phrase('informati_programare_aceptata'));
        ...
        $data['page_title']                 = get_phrase('cerere_programare');
        $this->load->view('backend/index', $data);
    }
    ...
    function prescription($task = "", $prescription_id = "", $menu_check = '', $patient_id = '')
    {
        ...
        if ($task == "create") {
            ...
            $this->session->set_flashdata('message', get_phrase('informati_reteta_salvata_cu_succes'));
            redirect(site_url('doctor/prescription'), 'refresh');
        }
        
        if ($task == "update") {
            $this->crud_model->update_prescription_info($prescription_id);
            $this->session->set_flashdata('message', get_phrase('informati_reteta_actualizata_cu_succes'));
        ...
        $data['page_name']         = 'manage_prescription';
        $data['page_title']        = get_phrase('reteta');
        $this->load->view('backend/index', $data);
    }
    ...
    function diagnosis_report($task = "", $diagnosis_report_id = "")
    {
        ...
        if ($task == "create") {
            ...
            $this->session->set_flashdata('message', get_phrase('informati_report_diagnostic_salvat_cu_succes'));
            redirect(site_url('doctor/prescription'), 'refresh');
        }
        
        if ($task == "delete") {
            $this->crud_model->delete_diagnosis_report_info($diagnosis_report_id);
            $this->session->set_flashdata('message', get_phrase('informati_report_diagnostic_sters_cu_succes'));
            redirect(site_url('doctor/prescription'), 'refresh');
        }
    }
    ...
    function message($param1 = 'message_home', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'send_new') {
            ...
            $this->session->set_flashdata('message', get_phrase('mesaj_trimis!'));
        ...
        $page_data['page_title']              = get_phrase('mesaje_private');
