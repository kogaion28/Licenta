...
class Home extends CI_Controller
{
    ...
    function home()
    {
        ...
        $page_data['page_title']      = get_phrase('Acasa');
    ...
    function about_us()
    {
        ...
        $page_data['page_title']      = get_phrase('Despre_noi');
        $page_data['service_section'] = $this->frontend_model->get_frontend_settings('service_section');
        $page_data['services']        = $this->frontend_model->get_services();
        $this->load->view('frontend/' . $this->theme . '/index', $page_data);
    }
    ...
    function appointment($doctor_id = '')
    {
        ...
        $page_data['page_title']  = get_phrase('programari');
        $page_data['departments'] = $this->frontend_model->get_departments();
    ...
    function make_an_appointment()
    {
        ...
        if ($result == 'success') {
            $this->session->set_flashdata('success_message', get_phrase('cerere_programare_cu_succes') . '. ' . get_phrase('conectați-vă_la_contul_dvs._pentru _a_vedea_dacă_acesta_este_aprobat'));
            redirect(site_url('home/appointment'), 'refresh');
        } else if ($result == 'captcha_failed') {
            $this->session->set_flashdata('error_message', get_phrase('captcha_verificare_esuata'));
            redirect(site_url('home/appointment'), 'refresh');
        } else if ($result == 'code_failed') {
            $this->session->set_flashdata('error_message', get_phrase('invalid_pacient_cod'));
            redirect(site_url('home/appointment'), 'refresh');
        } else if ($result == 'email_exists') {
            $this->session->set_flashdata('error_message', get_phrase('email-ul_exista_deja'));
            redirect(site_url('home/appointment'), 'refresh');
        }
    }
    ...
    function blog_details($blog_id = '')
    {
        ...
        $page_data['page_title'] = get_phrase('blog_detali');
    ...
    function contact_us($param1 = '')
    {
        if ($param1 == 'contact') {
            ...
            if ($result == true) {
                $this->session->set_flashdata('success_message', get_phrase('mesajul_tau_a_fost_trimis') . '.' . get_phrase('o_sa_luam_legatura_cu_tine'));
            } else {
                $this->session->set_flashdata('error_message', get_phrase('captcha_validare_esuata'));
            }
            redirect(site_url('home/contact_us'), 'refresh');
        }
        
        $page_data['page_name']  = 'contact_us';
        $page_data['page_title'] = get_phrase('contatacteaza-ne');
        $page_data['recaptcha']  = json_decode($this->frontend_model->get_frontend_settings('recaptcha'));
        $this->load->view('frontend/' . $this->theme . '/index', $page_data);
    }
