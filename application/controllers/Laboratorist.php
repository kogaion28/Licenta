...
class Laboratorist extends CI_Controller
{
    ...
    function index()
    {
        ...
        $data['page_title'] = get_phrase('laborant_pagina_principala');
    ...
    function blood_donor($task = "", $blood_donor_id = "")
    {
        ...
        if ($task == "create") {
            ...
            if ($blood_donor == null) {
                ...
                $this->session->set_flashdata('message', get_phrase('donator_de_sange_informatie_salvata_cu_succes'));
            } else {
                $this->session->set_flashdata('message', get_phrase('email_dublicat'));
            }
        ...
        if ($task == "update") {
            ...
            $this->session->set_flashdata('message', get_phrase('donator_de_sange_informatie_actualizata_cu_succes'));
            redirect(site_url('laboratorist/blood_donor'), 'refresh');
        }
        ...
        $data['page_title']       = get_phrase('donator_de_sange');
        $this->load->view('backend/index', $data);
    }
    ...
    function pathology_report()
    {
        ...
        $page_data['page_title'] = get_phrase('raport_de_patologie');
        $this->load->view('backend/index', $page_data);
    }
    function upload_pathology_report($param1 = "")
    {
        ...
        $this->session->set_flashdata('message', get_phrase('report_actualizat'));
