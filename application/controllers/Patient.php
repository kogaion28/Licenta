...
class Patient extends CI_Controller
{
    ...
    function index()
    {
        ...
        $data['page_title'] = get_phrase('pacient_pgina_principala');
    ...
    function admit_history($task = "")
    {
        ...
        $data['page_title'] = get_phrase('recunosc_istoricul');
        $this->load->view('backend/index', $data);
    }
    ...
    function operation_history($task = "")
    {
        ...
        $data['page_title'] = get_phrase('istoricul_operațiunilor');
        $this->load->view('backend/index', $data);
    }
    ...
    function manage_profile($task = "")
    {
        ...
        if ($task == "change_password") {
            ...
            if ($password == $old_password && $new_password == $confirm_new_password) {
                ...
                $this->session->set_flashdata('message', get_phrase('informati_parola_actulizate_cu_succes'));
                redirect(site_url('patient/manage_profile'), 'refresh');
            } else {
                $this->session->set_flashdata('message', get_phrase('actualizare_parola_estuat'));
                redirect(site_url('patient/manage_profile'), 'refresh');
            }
        }
    ...
    function appointment_pending($task = "", $appointment_id = "")
    {
        ...
        if ($task == "create") {
            ...
            $this->session->set_flashdata('message', get_phrase('cerere_pentru_programare_trimisa'));
        ...
        $data['page_title']               = get_phrase('asteptare_programare');
        $this->load->view('backend/index', $data);
    }
    ...
    function invoice($task = "", $invoice_id = "")
    {
        ...
        $data['page_title']   = get_phrase('factura_fiscala');
        $this->load->view('backend/index', $data);
    }
