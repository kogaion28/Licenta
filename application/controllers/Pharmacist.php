...
class Pharmacist extends CI_Controller
{
    ...
    function index()
    {
        ...
        $data['page_title'] = get_phrase('farmacist_pagina_principala');
    ...
    function medicine_category($task = "", $medicine_category_id = "")
    {
        ...
        if ($task == "create") {
            ...
            $this->session->set_flashdata('message', get_phrase('informati_categoria_medicamentelor_salvata_cu_succes'));
            redirect(site_url('pharmacist/medicine_category'), 'refresh');
        }
        
        if ($task == "update") {
            $this->crud_model->update_medicine_category_info($medicine_category_id);
            $this->session->set_flashdata('message', get_phrase('informati_categoria_medicamentelor_actualizata_cu_succes'));
            redirect(site_url('pharmacist/medicine_category'), 'refresh');
        }
        ...
        $data['page_title']             = get_phrase('categoria_medicamentelor');
        $this->load->view('backend/index', $data);
    }
    ...
    function medicine($task = "", $medicine_id = "")
    {
        ...
        if ($task == "create") {
            ...
            $this->session->set_flashdata('message', get_phrase('informati_medicamente_salvate_succes'));
            redirect(site_url('pharmacist/medicine'), 'refresh');
        }
        
        if ($task == "update") {
            $this->crud_model->update_medicine_info($medicine_id);
            $this->session->set_flashdata('message', get_phrase('informati_medicamente_actualizate_cu_succes'));
            redirect(site_url('pharmacist/medicine'), 'refresh');
        }
    ...
    function manage_profile($task = "")
    {
        ...
        if ($task == "change_password") {
            ...
            if ($password == $old_password && $new_password == $confirm_new_password) {
                ...
                $this->session->set_flashdata('message', get_phrase('actualizare_parola_esuat'));
                redirect(site_url('pharmacist/manage_profile'), 'refresh');
            }
        }
    ...
    function payroll_list($param1 = '', $param2 = '')
    {
        ...
        $page_data['page_title'] = get_phrase('lista_salarizare');
    ...
    function medicine_sale($task = "", $param2 = "")
    {
        ...
        if ($task == "create") {
            ...
            $this->session->set_flashdata('message', get_phrase('data_adaugata_cu_succes'));
            redirect(site_url('pharmacist/medicine_sale'), 'refresh');
        }
        
        $data['page_name']  = 'medicine_sale';
        $data['page_title'] = get_phrase('vanzari_medicamente');
        $this->load->view('backend/index', $data);
    }
    ...
    function create_medicine_sale($task = "", $param2 = "")
    {
        ...
        $data['page_title'] = get_phrase('adauga_vanzari_medicamente');
        $this->load->view('backend/index', $data);
    }
