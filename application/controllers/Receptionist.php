...
class Receptionist extends CI_Controller
{
    ...
    function index()
    {
        ...
        $data['page_title'] = get_phrase('receptionist_pagina_principala');
    ...
    function appointment($task = "", $doctor_id = 'all', $start_timestamp = "", $end_timestamp = "")
    {
        ...
        if ($task == "create") {
            ...
            $this->session->set_flashdata('message', get_phrase('informati_programeri_salvate_cu_succes'));
        ...
        $data['page_title']       = get_phrase('programari');
        $this->load->view('backend/index', $data);
    }
    ...
    function appointment_requested($task = "", $appointment_id = "")
    {
        ...
        if ($task == "approve") {
            ...
            $this->session->set_flashdata('message', get_phrase('informati_programari_aprobat'));
    ...
    function manage_profile($task = "")
    {
        ...
        if ($task == "change_password") {
            ...
            if ($password == $old_password && $new_password == $confirm_new_password) {
                ...
                $this->session->set_flashdata('message', get_phrase('informati_programare_actualizate_cu_succes'));
                redirect(site_url('receptionist/manage_profile'), 'refresh');
            } else {
